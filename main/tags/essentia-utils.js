		...
		handleListArr.slice(count * numTracks, currMax).forEach((handle, i) => {
			...
			if (_isFile(path)) {
				...
				if (data) {
					...
					const tag = (data.match(rgexKey) || [,])[1]; // NOSONAR
					// eslint-disable-next-line no-sparse-arrays
					const tagScale = (data.match(rgexScale) || [,])[1]; // NOSONAR
		...
		handleListArr.slice(count * numTracks, currMax).forEach((handle, i) => {
			...
			if (_isFile(path)) {
				...
				if (data) {
					...
					tagName.forEach((key) => {
						if (key === 'BPM' && Object.hasOwn(data, 'rhythm')) {
							val = data.rhythm.bpm;
							if (Number.isFinite(val)) { tag[key] = Math.round(val); }
						} else if (key === 'KEY' && Object.hasOwn(data, 'tonal')) {
						...
						} else if (key === 'DANCENESS' && Object.hasOwn(data, 'rhythm')) {
							val = data.rhythm.danceability;
							if (Number.isFinite(val)) { tag[key] = round(val, 2); }
						} else if (key === 'LRA' && Object.hasOwn(data, 'lowlevel') && Object.hasOwn(data.lowlevel, 'loudness_ebu128')) {
