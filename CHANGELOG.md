...
- Tagging\Batch tagger: added support for [foo_dr_meter](https://foobar.hyv.fi/?view=foo_dr_meter) replacement of all the DR meters out there, compatible with x64 and much faster. 100% clones the log and tags output.
- Tagging\Batch tagger: added quiet mode settings. On quiet mode they can be automatically run without user input (usually on report windows). Note you also have to enable quiet mode on their own component settings (ask for support in their respective threads if you need help). Enabling quiet mode on the components without also enabling it on the script will probably produce errors while tagging files due to them being in use; you must use the same settings at both places.
- Tagging\Batch tagger: added configurable tags for every tool. Note in some cases tags also have to be set on their own component settings (ask for support in their respective threads if you need help).
- Tagging\Batch tagger: added configurable menu entries for every tool. These are related to the contextual menu called when running to add/remove tags.
- Tagging\Batch tagger: added setting to suppress warning popup when running.
...
- Tagging\Check tags: input popups to set dictionary now display the current list of dictionaries found (more may be installed by the user).
- Other tools\Write Tags: renamed to 'Tagging\Batch Tagger'.
- Tagging\Batch tagger: now uses the global tag remap set by user ('[foobar_profile]\foobar2000\js_data\presets\global\globTags.json') for danceness and LRA tags.
- Tagging\Batch tagger: Shift + L. Click action on button has been moved to Ctrl. + L. Click; for consistency with the other buttons which have shift actions associated to settings menus (not available on this one).
- Tagging\Batch tagger: added more checks for incompatible tools or settings.
- Tagging\Batch tagger: updated for compatibility with latest True Peak Scanner updates.
