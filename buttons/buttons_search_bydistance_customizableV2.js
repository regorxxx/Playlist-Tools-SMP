...
var newButtons = {
    SimilarUserSet: new SimpleButton(calcNextButtonCoordinates(buttonCoordinates, buttonOrientation, buttonOrientation == 'x' ? true : false).x, calcNextButtonCoordinates(buttonCoordinates, buttonOrientation, buttonOrientation == 'x' ? false : true).y, buttonCoordinates.w, buttonCoordinates.h, getProperties(newButtonsProperties, prefix)['customName'], function () {
		let t0 = Date.now();
		let t1 = 0;
		let {genreWeight , styleWeight , dyngenreWeight , moodWeight , keyWeight , dateWeight , dateRange , bpmWeight , bpmRange , playlistLength , probPick , scoreFilter , sbd_max_graph_distance , bRandomPick , method , forcedQuery , bSortRandom , genreTag , styleTag , moodTag, customName} = getPropertiesPairs(this.buttonsProperties, this.prefix, undefined, undefined, true); //This gets all the panel properties at once. Note the names are the keys, they don't need to be sorted!
		if (customName == 'Customize!') {
			...
			do_searchby_distanceV2(genreWeight , styleWeight , dyngenreWeight , moodWeight , keyWeight , dateWeight , dateRange , bpmWeight , bpmRange , playlistLength , probPick , scoreFilter , sbd_max_graph_distance , bRandomPick , method , forcedQuery , bSortRandom, this.prefix, this.buttonsProperties);
		} // All set according to properties panel!
		t1 = Date.now();
