...

{
	const newButton = {
		'Main Menu': new ThemedButton({ x: 0, y: 0, w: _gr.CalcTextWidth(newButtonsProperties.customName[1], _gdiFont(globFonts.button.name, globFonts.button.size * buttonsBar.config.scale)) + 30 * _scale(1, false) / _scale(buttonsBar.config.scale), h: 22 }, newButtonsProperties.customName[1], function (mask) {
			...
			return info;
		}, prefix, newButtonsProperties, newButtonsProperties.icon[1], void (0),
		{
			defText: 'Main menu shortcut',
			stateCheck: (parent) => {
				const entries = JSON.parse(parent.buttonsProperties.entries[1]);
				...
				if (new Set(results.map((e) => e.check)).size !== 1) {
					fb.ShowPopupMessage(
						'Main menu button: ' + parent.buttonsProperties.customName[1] +
						'\nEntries:' +
		...
		(parent) => { // Default state on init
			...
			let bActive;
			if (indicator.menuCheck) {
				bActive = parent.stateCheck();
			} else {
				bActive = parent.buttonsProperties.state[1];
			...
			if (indicator.enabled && indicator.init) { parent.switchActive(bActive); }
